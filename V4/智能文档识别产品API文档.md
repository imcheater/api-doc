...
| fileFormat | string | 要检测的文档格式 | N | 可选值：<br/>`DOCX`<br/>`PDF`<br/>`DOC`<br/>`XLS`<br/>`XLSX`<br/>`XLSM`<br/>`XLTX`<br/>`XLTM`<br/>`XLSB`<br/>`CSV`<br/>`PPT`<br/>`PPTX`<br/>`PPS`<br/>`PPSX`<br/>`TXT`<br/>若不传或传空值，则默认按网页链接或文本内容检测<br/>若fileFormat与文档实际格式不一致，则返回报错参数错误<br/> |
